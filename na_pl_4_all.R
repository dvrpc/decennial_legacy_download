...
# -----------------------------
header_file_path <- "G:\\Shared drives\\Data Coordination\\2020 Census\\2020 redistricting legacy format\\NJ\\njgeo2020.pl"
part1_file_path  <- "G:\\Shared drives\\Data Coordination\\2020 Census\\2020 redistricting legacy format\\NJ\\nj000012020.pl"
part2_file_path  <- "G:\\Shared drives\\Data Coordination\\2020 Census\\2020 redistricting legacy format\\NJ\\nj000022020.pl"
part3_file_path  <- "G:\\Shared drives\\Data Coordination\\2020 Census\\2020 redistricting legacy format\\NJ\\nj000032020.pl"

# -----------------------------
# Import the data
# -----------------------------
header <- read.delim(header_file_path, header=FALSE, colClasses="character", sep="|")
part1NJ  <- read.delim(part1_file_path,  header=FALSE, colClasses="character", sep="|")
part2NJ  <- read.delim(part2_file_path,  header=FALSE, colClasses="character", sep="|")
part3NJ  <- read.delim(part3_file_path,  header=FALSE, colClasses="character", sep="|")
...
colnames(header) <- c("FILEID", "STUSAB", "SUMLEV", "GEOVAR", "GEOCOMP", "CHARITER", "CIFSN", "LOGRECNO", "GEOID", 
                      ...
                      "LSADC", "PARTFLAG", "UGA")
colnames(part1NJ) <- c("FILEID", "STUSAB", "CHARITER", "CIFSN", "LOGRECNO", 
                     paste0("P00", c(10001:10071, 20001:20073)))
colnames(part2NJ) <- c("FILEID", "STUSAB", "CHARITER", "CIFSN", "LOGRECNO", 
                     paste0("P00", c(30001:30071, 40001:40073)), 
                     paste0("H00", 10001:10003))
colnames(part3NJ) <- c("FILEID", "STUSAB", "CHARITER", "CIFSN", "LOGRECNO",
                     paste0("P00", 50001:50010))

# -----------------------------
# Merge the data
# -----------------------------
combine <- Reduce(function(x,y) {merge(x, y, by=c("LOGRECNO", "STUSAB", "FILEID", "CHARITER"))}, list(header[,-7], part1NJ[,-4], part2NJ[,-4], part3NJ))

# -----------------------------
# Order the data
# -----------------------------
combineNJ <- combine[order(combine$LOGRECNO), c("FILEID", "STUSAB", "SUMLEV", "GEOVAR", "GEOCOMP", "CHARITER", "CIFSN", "LOGRECNO", "GEOID", 
                                              "GEOCODE", "REGION", "DIVISION", "STATE", "STATENS", "COUNTY", "COUNTYCC", "COUNTYNS", "COUSUB",
                                              "COUSUBCC", "COUSUBNS", "SUBMCD", "SUBMCDCC", "SUBMCDNS", "ESTATE", "ESTATECC", "ESTATENS", 
                                              "CONCIT", "CONCITCC", "CONCITNS", "PLACE", "PLACECC", "PLACENS", "TRACT", "BLKGRP", "BLOCK", 
                                              "AIANHH", "AIHHTLI", "AIANHHFP", "AIANHHCC", "AIANHHNS", "AITS", "AITSFP", "AITSCC", "AITSNS",
...
rownames(combineNJ) <- 1:nrow(combineNJ)
